...
public class CreateOrderKafkaMessagePublisher implements OrderCreatedPaymentRequestMessagePublisher {
    ...
    public void publish(OrderCreatedEvent domainEvent) {
        ...
        try {
            final var paymentRequestAvroModel = orderMessagingDataMapper
                    .orderCreatedEventToPaymentRequestAvroModel(domainEvent);

            kafkaProducer.send(orderServiceConfigData.getPaymentRequestTopicName(), orderId,
                    paymentRequestAvroModel, orderKafkaMessageHelper.getKafkaCallback(
                            orderServiceConfigData.getPaymentResponseTopicName(), paymentRequestAvroModel,
                            orderId, paymentRequestAvroModel.getClass().getSimpleName()));

            log.info("{} sent to Kafka for order id: {}", paymentRequestAvroModel.getClass().getSimpleName(),
